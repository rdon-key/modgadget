...
func TestMeasureLineEmptyStringIncludesMetrics(t *testing.T) {
	face := spanFace(FontMetrics{Ascent: 8, Descent: 2, LineGap: 1}, nil, "")
...
func TestMeasureLineSelectsMaximumMetricsAndMeasuresInk(t *testing.T) {
	faceA := spanFace(FontMetrics{Ascent: 8, Descent: 1, LineGap: 0}, []testGlyphInfo{
		{Rune: 'a', Width: 1, Height: 14, AdvanceX: 4, BearingY: 10},
	}, strings.Repeat("\x00", 14))
	faceB := spanFace(FontMetrics{Ascent: 6, Descent: 3, LineGap: 1}, []testGlyphInfo{
		{Rune: 'b', Width: 1, Height: 1, AdvanceX: -2, BearingX: -1},
	}, "\x00")
	lineGapFace := spanFace(FontMetrics{Ascent: 7, Descent: 2, LineGap: 2}, nil, "")
...
func TestMeasureLineSelectsMaximumNegativeMetrics(t *testing.T) {
	faceA := spanFace(FontMetrics{Ascent: -5, Descent: -2, LineGap: -4}, nil, "")
	faceB := spanFace(FontMetrics{Ascent: -3, Descent: -6, LineGap: -1}, nil, "")
...
func TestMeasureLineWhitespaceHasAdvanceWithoutInk(t *testing.T) {
	face := spanFace(FontMetrics{Ascent: 5, Descent: 2, LineGap: 1}, []testGlyphInfo{{Rune: ' ', AdvanceX: 3}}, "")
	got, err := MeasureLine([]Span{{Font: face, Value: "  "}})
...
func TestMeasureLineValidationPartialResults(t *testing.T) {
	first := spanFace(FontMetrics{Ascent: 2, Descent: 1}, []testGlyphInfo{{Rune: 'a', AdvanceX: 3}}, "")
	second := spanFace(FontMetrics{Ascent: 4, LineGap: 1}, []testGlyphInfo{{Rune: 'b', AdvanceX: 2}}, "")
...
func TestMeasureLineGlyphAndHorizontalOverflow(t *testing.T) {
	maxAdvance := spanFace(FontMetrics{Ascent: 1}, []testGlyphInfo{{Rune: 'a', AdvanceX: math.MaxInt16}}, "")
	coordinate := spanFace(FontMetrics{Descent: 1}, []testGlyphInfo{{Rune: 'b', Width: 1, Height: 1, BearingX: 1}}, "\x00")
	advance := spanFace(FontMetrics{Descent: 1}, []testGlyphInfo{{Rune: 'c', AdvanceX: 1}}, "")
	tests := []struct {
		name string
		face Font
		char string
...
func TestMeasureLineAdvanceYOverflowReturnsPriorLine(t *testing.T) {
	first := spanFace(FontMetrics{Ascent: math.MaxInt16}, nil, "")
	second := spanFace(FontMetrics{Descent: 1}, nil, "")
